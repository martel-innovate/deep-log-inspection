...
if [ -z "$DN" ]; then
   DN="CN=elastic.deeplogmanager.lab.fiware.org, OU=SSL, O=FIWARE, L=Madrid, C=ES"
fi
...
"$BIN_PATH" -genkey \
        ...
        -storepass $KS_PASS \
        -dname "$DN" \
        -ext san=dns:elastic.deeplogmanager.lab.fiware.org,dns:localhost,ip:127.0.0.1,oid:1.2.3.4.5.5 
